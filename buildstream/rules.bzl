...
def _bst_element_impl(ctx):
    ...
    elif ctx.file.build_file:
        ctx.actions.declare_symlink("//:BUILD")
        ctx.actions.symlink("//:BUILD", ctx.file.build_file)
    elif ctx.attr.build_file_content:
        ctx.actions.write("BUILD", content = ctx.attr.build_file_content)
    else:
        ctx.actions.declare_symlink("BUILD")
        ctx.actions.symlink("BUILD", "//buildstream:BUILD.pkg")
    ...
    project_conf = ctx.file.repository
    project = project_conf.dirname
